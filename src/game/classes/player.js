...
export class Player extends Sprite {
  constructor({ imgEl, laserImg, width, height, position, rotation, thrust, playerType, isAi, game }) {
  ...
  updateLaserPositions(ctx) {
    ...
    laserKeys &&
      laserKeys.forEach((laser) => {
        ...
        if (laserValue.position.y < -outOfBounds || laserValue.position.y > this.game.height + outOfBounds || laserValue.position.x < -outOfBounds || laserValue.position.x > innerWidth + outOfBounds) {
  ...
  handleCanvasBoundaries() {
    if ((this.position.y <= this.boundaryPadding && Math.sign(this.velocity.y) === -1) || (this.position.y >= this.game.height - this.boundaryPadding && Math.sign(this.velocity.y) === 1)) {
      this.velocity.y = (this.velocity.y / 2) * -1;
    }

    if ((this.position.x <= this.boundaryPadding && Math.sign(this.velocity.x) === -1) || (this.position.x >= innerWidth - this.boundaryPadding && Math.sign(this.velocity.x) === 1)) {
  ...
  radarDetector(dinosaurs) {
    ...
    let xToCenter = Math.abs(this.position.x - innerWidth / 2) - innerWidth / 2 + this.boundaryPadding;
    let yToCenter = Math.abs(this.position.y - this.game.height / 2) - this.game.height / 2 + this.boundaryPadding;
    ...
    if (dinoValues.length) {
      ...
      const dinoDistance = Math.hypot(this.position.x - dino.position.x, this.position.y - dino.position.y);
  ...
  render(ctx) {
    ...
    if (this.controls.thrust) {
      ...
      ctx.translate(-this.width / 5.2, this.height / 2); //translate the canvas to draw the thrust

      //create the gradient for the thrust
      let grd = ctx.createLinearGradient(this.width / 2 / 2, 0, this.width / 2 / 2, this.height / 2.5);
