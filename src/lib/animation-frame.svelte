...
<script>
	...
	function updateProgress() {
		...
		if (start <= 0) {
			let frameLeft = height + start; //Addition is not intuitive here but we do this because we recieve a negative value from y when we enter into the frame. So gets the amount of the frame we have left to scroll trhough if px.
